...
class AWWSM4_HIER_AE:
	...
	def	generate_AE_one_by_one(self):
		for i_AE in range(self.n_sub_net):
			...
			print("Doing the %dth sub autoencoder." % n_AE)
			...
			if i_AE == 0:
				init_HIER_AE_cmd = "Autoencoder_hier_%d(%s, %s)" % (n_AE, encoder_name, decoder_name) 
			elif i_AE > 0:
				init_HIER_AE_cmd = "Autoencoder_hier_%d(%s, %s, %s)" % \
												(n_AE, encoder_name, decoder_name, HIER_AE_name_prev)
			...
			model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_filepath,
                                                               ...
                                                               save_freq=10*40)
			print("Preparation done! Do the training now!")
			#do fit
